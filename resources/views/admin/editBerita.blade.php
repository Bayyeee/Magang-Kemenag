            ...
            <div class="content">
                ...
                <div class="intro-y grid grid-cols-12 gap-6 mt-5">
                        ...
                        <div class="intro-y col-span-12 md:col-span-6 xl:col-span-4 box">
                            <div class="flex items-center border-b border-slate-200/60 dark:border-darkmode-400 px-5 py-4">
                                <div class="ml-3 mr-auto">
                                    <a href="" class="font-medium">{{ $item->judul_berita }}</a>
                                </div>
                                    ...
                                    <div class="dropdown-menu w-40">
                                        <ul class="dropdown-content">
                                            <li>
                                                ...
                                                <a href="javascript:;" class="dropdown-item" data-tw-toggle="modal" data-tw-target="#edit-confirmation-modal"
                                                    data-berita-id="{{ $item->id_berita }}" onclick="editBerita({{ $item->id_berita }})">
                            ...
                            <div class="p-5">
                                <div class="h-40 2xl:h-56 image-fit">
                                    <img alt="{{ $item->judul_berita }}" data-action="zoom" class="rounded-md zoom-in" src="{{ $item->fotoBerita->path }}">
                                </div>
                                ...
                                <div class="text-slate-600 dark:text-slate-500 mt-2">
                                    @if (strlen($item->isi_berita) > 50)
                                        <span id="isi-berita-short-{{ $item->id_berita }}" class="isi-berita-short">{{ substr($item->isi_berita, 0, 50) }}...</span>
                                        <span id="isi-berita-full-{{ $item->id_berita }}" class="isi-berita-full" style="display: none;">{{ $item->isi_berita }}</span>
                                        <a href="#" class="toggle-isi-berita text-primary" data-id="{{ $item->id_berita }}">Baca Selengkapnya</a>
                                    @else
                                        {{ $item->isi_berita }}
                                    @endif
                                </div>
                            ...
                            <div class="flex items-center px-5 py-3 border-t border-slate-200/60 dark:border-darkmode-400">
                                <a href="#" class="intro-x w-8 h-8 flex items-center justify-center rounded-full border border-slate-300 dark:border-darkmode-400 dark:bg-darkmode-300 dark:text-slate-300 text-slate-500 mr-2 tooltip" title="{{ $item->user->role }}">
                                    <i data-lucide="users" class="w-3 h-3"></i>
                                </a>
                                <a href="#" class="intro-x w-8 h-8 flex items-center justify-center rounded-full border border-slate-300 dark:border-darkmode-400 dark:bg-darkmode-300 dark:text-slate-300 text-slate-500 mr-2 tooltip" title="{{ date('d M Y ', strtotime($item->waktu_upload)) }}">
                            ...
                            <div class="box">
                                <div class="p-5">
                                    <div class="text-center">
                                        <h3 class="text-xl font-medium">Tidak ada berita yang tersedia.</h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="modal-body p-0">
                                        ...
                                        <div class="px-5 pb-8">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="edit_judul_berita" name="judul_berita" class="form-control" maxlength="30" value="" required>
                                            </div>
                                            <div class="form-group mt-3">
                                                <label for="edit_isi_berita">Isi Berita</label>
                                                <textarea id="edit_isi_berita" name="isi_berita" class="form-control w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out" maxlength="300" required></textarea>
                                            </div>
                                            <div class="form-group mt-3">
                                                <label for="edit_foto">Upload Foto</label>
                                                <input type="file" id="edit_foto" name="foto" class="border form-control" accept="image/*">
                                            </div>
                                        </div>
                                        <div class="px-5 pb-8 text-center">
                                            <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-24 mr-1">Batal</button>
                                            <button type="submit" class="btn btn-outline-primary w-24">Simpan</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                    ...
                    <div id="tambah-confirmation-modal" class="modal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form id="tambahForm" action="{{ route('tambah-berita') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="modal-body p-0">
                                        <div class="p-5 text-center">
                                        ...
                                        <div class="px-5 pb-8">
                                            ...
                                            <div class="form-group mt-3">
                                                <label for="edit_isi_berita">Isi Berita</label>
                                                <textarea id="edit_isi_berita" name="isi_berita" class="form-control w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out" maxlength="500" required></textarea>
                                            </div>
                                            <div class="form-group mt-3">
                                                <label for="edit_foto">Upload Foto</label>
                                                <input type="file" id="edit_foto" name="foto" class="border form-control" accept="image/*">
